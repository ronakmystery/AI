...
def classify_single_image(image):
    """Classifies a single image while ensuring correct format."""
    
    # 🔹 Convert OpenCV (BGR) to PIL (RGB) if image is a NumPy array
    if isinstance(image, np.ndarray):
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
        image = Image.fromarray(image)  # Convert NumPy to PIL
    
    elif not isinstance(image, Image.Image):
        raise TypeError(f"Expected a PIL Image or NumPy array, but got: {type(image)}")
    ...
    classname=class_map[predicted_label]

    return classname # Return predicted class
